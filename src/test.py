...
from datasets import ClassifyDataset, VerifyDataset, ClassifyWithSimilarity, DeepIdDataset
...
def test_deepid_net(configer):
    ...
    testloader = DataLoader(testset, configer.batchsize, shuffle=False)
    model = DeepID(type='deepid'); model.load()
    metric = VerifyBinLoss()
    ...
    print_log = "{} || test | loss: {:.4f} acc: {:.2%} p: {:.4f}".\
        format(getTime(), loss_test, acc_test, np.exp(-loss_test))
    print(print_log)
