...
def cls_ohem(cls_prob, label):
    ...
    label_filter_invalid = tf.where(tf.less(label,0), zeros, label) # tf.where(input, a, b): 将a中对应input中true的位置的元素值不变，其余元素进行替换，替换成b中对应位置的元素值
    label_int = tf.cast(label_filter_invalid, tf.int32)
    # reshape cls_prob
    num_cls_prob = tf.size(cls_prob)    # returns the number of elements
    cls_prob_reshape = tf.reshape(cls_prob,[num_cls_prob,-1])   # 展开为1维向量，维度为2
...
def P_Net(inputs,label=None,bbox_target=None,landmark_target=None,training=True):
    ...
    with slim.arg_scope([slim.conv2d],
        ...
        #test
            ...
            _activation_summary(cls_pro_test)
            _activation_summary(bbox_pred_test)
            _activation_summary(landmark_pred_test)
